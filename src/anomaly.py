...
def compute_text_features(
    ...
    if total:
        try:
            ...
            digits_str = str(total_val).replace(".", "").lstrip("0")
            if digits_str:
                first_digit = int(digits_str[0])
                features["total_leading_digit"] = float(first_digit)
                benford = [0.301, 0.176, 0.125, 0.097, 0.079, 0.067, 0.058, 0.051, 0.046]
                if 1 <= first_digit <= 9:
                    features["total_benford_dev"] = float(1.0 - benford[first_digit - 1])
                else:
                    features["total_benford_dev"] = 0.0
            else:
                features["total_leading_digit"] = 0.0
                features["total_benford_dev"] = 0.0
    ...
    if ocr_results:
        ...
        features["ocr_conf_mean"] = float(np.mean(confidences)) if confidences else 0.0
        features["ocr_conf_min"] = float(np.min(confidences)) if confidences else 0.0
        features["ocr_conf_std"] = float(np.std(confidences)) if confidences else 0.0
        features["ocr_low_conf_ratio"] = float(
            sum(1 for c in confidences if c < 0.3) / max(len(confidences), 1)
        )
        features["ocr_token_count"] = float(len(confidences))
    else:
        features["ocr_conf_mean"] = 0.0
        features["ocr_conf_min"] = 0.0
        features["ocr_conf_std"] = 0.0
        features["ocr_low_conf_ratio"] = 0.0
        features["ocr_token_count"] = 0.0
...
def build_feature_vector(
    ...
    if image_path and os.path.exists(image_path) and os.path.getsize(image_path) > 0:
        try:
            ...
            features["is_corrupt_image"] = 0.0
        except Exception:
            features["is_corrupt_image"] = 1.0
    else:
        features["is_corrupt_image"] = 1.0

    text_feats = compute_text_features(fields, ocr_text, ocr_results)
...
class AnomalyDetector:
    ...
    def predict(self, features_list: list[dict[str, float]]) -> list[int]:
        ...
        if self.threshold != 0.5:
            ...
            return [int(p >= self.threshold) for p in probas]
    ...
    def save(self, model_dir: str) -> None:
        ...
        meta = {
            ...
            "train_f1": getattr(self, "_train_f1", 0.0),
