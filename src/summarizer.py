...
def generate_anomaly_summary(
    ...
    if gradient_skew > 3.0 or gradient_skew < -2.0:
        reasons.append(
    ...
    ocr_conf_mean = features.get("ocr_conf_mean", 1.0)
    if ocr_conf_mean < 0.4 and is_forged == 1:
        reasons.append(
            f"Low OCR confidence (mean={ocr_conf_mean:.2f}). "
            "The text in this document is unusually difficult to read, which can "
            "indicate image manipulation that disrupted text clarity."
        )
